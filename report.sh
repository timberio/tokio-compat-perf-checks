...
print_dstat_stat() {
  ...
  local HEADER_NUM="$2"
  echo "dstat ${HEADER}:"
  for DSTAT_FILE in "$RESULTS_DIR"/dstat-*.csv; do
    DSTAT_RECV="$(./compare-dstat-val.sh "$HEADER_NUM" "$DSTAT_FILE")"
    NAME="${DSTAT_FILE}"
    NAME="${NAME/"$RESULTS_DIR"\/dstat-/}"
    NAME="${NAME/.csv/}"
...
for TEST_SERVER_SUMMARY_FILE in "$RESULTS_DIR"/tcp_test_server_summary-*.json; do
  MESSAGE_COUNT="$(jq '.message_count' < "$TEST_SERVER_SUMMARY_FILE")"
  NAME="${TEST_SERVER_SUMMARY_FILE}"
  NAME="${NAME/"$RESULTS_DIR"\/tcp_test_server_summary-/}"
  NAME="${NAME/.json/}"
